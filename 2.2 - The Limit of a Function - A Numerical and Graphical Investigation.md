...
>1. If the values of $f(x)$ increase without bound as the values of $x$ (where $x<a$) approach the number $a$, then we say that the limit as $x$ approaches $a$ from the left is positive infinity and we write: $$\lim_{x\to a^-}=+\infty$$
