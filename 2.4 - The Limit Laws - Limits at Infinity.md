...
>
>**Proof:**
>Let $p$ be a rational number. The only constraint on using the Limit Laws is that the values of the limits must be finite. Therefore, $a$ is allowed to be $\pm\infty$. Thus: $$\lim_{x\to\infty}\dfrac{1}{x^p} = \lim_{x\to\infty} \bigg(\dfrac{1}{x})^p\bigg)$$
